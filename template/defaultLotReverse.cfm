<!--- Emaze Auction version 2.1, 1.01 / Tuesday, August 17, 1999 --->
...
<TR BGCOLOR="#navTopTableBg#"><TD ALIGN=right><FONT SIZE=2>Quantity Requested: </FONT></TD>
...
<CFOUTPUT><TR BGCOLOR="#navTopTableBg#"></CFOUTPUT><TD ALIGN=right NOWRAP><FONT SIZE=2><B>Maximum winning bid: </B></TD>
...
<CFELSE>
	...
	<CFQUERY NAME=getWinningBids DATASOURCE="#EAdatasource#">
		SELECT Member.username, Member.feedbackRating, Bid.bidPrice, Bid.bidQuantityWin, Bid.bidEditDateTime
			<CFIF reverseBidDescription EQ 1 AND reverseBidDescriptionDisplay EQ 1>, Bid.bidDescription</CFIF>
			<CFIF reverseBidDisplayAllBids EQ 1>, Bid.bidWin</CFIF>
		FROM Bid INNER JOIN Member ON Bid.userID = Member.userID
		WHERE Bid.lotID = #lotID#<CFIF reverseBidDisplayAllBids EQ 0> AND Bid.bidWin = 1</CFIF>
		ORDER BY Bid.bidPrice
	</CFQUERY>
	...
	<CFELSE>
		...
		<TH BGCOLOR="#navTopTableBg#">Bid <CFIF reverseBidDescription EQ 1 AND reverseBidDescriptionDisplay EQ 1><FONT SIZE=2>(description)</FONT></CFIF></TH>
		...
		<CFOUTPUT QUERY=getWinningBids>
			...
			<TD ALIGN=right>#LSCurrencyFormat(bidPrice,"local")#
			<CFIF reverseBidDescription EQ 1 AND reverseBidDescriptionDisplay EQ 1>
				<CFIF bidDescription NEQ "" AND bidDescription NEQ " "><FONT SIZE=2>(<A HREF="#systemURL#/program/bidDescription.cfm?lotID=#lotID#&username=#URLEncodedFormat(username)#">description</A>)</FONT></CFIF>
			</CFIF></TD>
...
<CFIF lotStatus EQ "Open">
	...
	<CFELSE>
		<CFINCLUDE TEMPLATE="../program/reverseBidForm.cfm">
	</CFIF>
<CFELSE><!--- lotStatus EQ "Closed" --->
